...
/// Action returned by the scan callback.
pub enum ScanAction {
    Continue,
    Stop,
}
...
impl GenshinRepositoryScanController {
    /// Run the scan loop, calling `callback` after each item is clicked and panel loaded.
    /// The callback should capture/process the current panel and return `ScanAction`.
    /// This replaces the old coroutine-based `get_generator`.
    pub fn run_scan<F>(&mut self, item_count: usize, mut callback: F) -> Result<ReturnResult>
    where
        F: FnMut(&Self) -> ScanAction,
    {
        let mut scanned_row = 0;
        let mut scanned_count = 0;
        let mut start_row = 0;

        let total_row = (item_count + self.col - 1) / self.col;
        let last_row_col = if item_count % self.col == 0 {
            self.col
        } else {
            item_count % self.col
        };
        ...
        self.sample_initial_color().unwrap();

        let row = self.row.min(total_row);
        ...
        'outer: while scanned_count < item_count {
            for cur_row in start_row..row {
                ...
                for col in 0..row_item_count {
                    ...
                    self.move_to(cur_row, col);
                    self.system_control.mouse_click().unwrap();

                    #[cfg(target_os = "macos")]
                    utils::sleep(20);

                    let _ = self.wait_until_switched();

                    // Call the callback (replaces coroutine yield)
                    match callback(self) {
                        ScanAction::Continue => {}
                ...
                if scanned_row >= self.config.max_row as usize {
                    info!("到达最大行数，准备退出……");
                    break 'outer;
                }
            }
    ...
    pub fn mouse_scroll(&mut self, length: i32, try_find: bool) {
        ...
        #[cfg(target_os = "macos")]
        {
            match self.game_info.ui {
                yas::game_info::UI::Desktop => {
                    self.system_control.mouse_scroll(length);
                    utils::sleep(20);
                }
                yas::game_info::UI::Mobile => {
                    if try_find {
